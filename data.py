...
def compute_token_to_index(vocabulary: Tuple[str], counts: Tuple[int]) -> Tuple[Dict[str, int], Dict[int, str]]:
    ...
    for i in range(len(vocabulary)):
        dict1[vocabulary[i]]=i
        dict2[i]=vocabulary[i]
        dict3[i]=counts[i]
...
def get_token_pairs_from_window(sequence: Sequence[str], window_size: int, token_to_index: Dict[str, int]) -> Sequence[Tuple[int, int]]:
    ...
    for i, center_token in enumerate(sequence):
        center_token_index = token_to_index.get(center_token)
        if center_token_index is not None:
            for j in range(i - window_size, i + window_size + 1):
                if j >= 0 and j < len(sequence) and i != j:
                    context_token = sequence[j]
                    context_token_index = token_to_index.get(context_token)
                    if context_token_index is not None:
                        pairs.append((center_token_index, context_token_index))
    return pairs
    ###########################
